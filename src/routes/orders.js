...
router.post('/', isAuthenticated, async (req, res) => {
    try {
        ...
        for (const cartItem of items) {
            ...
            // }
            let basePrice, availableStock, variantId = null, discountActive, discountPercent;
            ...
            const unitPrice = (discountActive && discountPercent > 0)
        ...
        await createNotification(
            'new_order',
            `New order started by ${fullname} (₦${total.toLocaleString()})`,
            `/admin/orders/`,
            order._id
        );
